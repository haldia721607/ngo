...
//For if we add textbox on js grid and want to edit than js is refreshed then we use this filteOnKeyPress method
 //scope.filteOnKeyPress = function (grid) {
        //$("jsgrid-filter-row :input", $(grid)).keydown(function () {
           // var self = this;
          //  if (self.timeout)
               // clearTimeout(self.timeout);
...
var hplc = (function (scope) {
    scope.ajaxCall = function (method, url, data, dataType, f, headers = null, asyncHit = true,showWaiting=true, traditional = false,contentType="application/x-www-form-urlencoded; charset=UTF-8;",processData=true) {
        $.ajax({
            type: method,
            url: url,
            data: data,
